...
public class Board {
    ...
    //Os homens são coloridos como vermelho e azul para evitar as convenções de nomenclatura.
    //Os nomes de cada time/jogador que são mostrados aos jogadores são alterados através de "Trilha.Color" e são independentes dos nomes usados aqui.
    public ArrayList<Man> red = new ArrayList<Man>();
    public ArrayList<Man> blue = new ArrayList<Man>();
    ...
    /*
     * Lógica da tabuleiro principal.
     * A conexao das casas e seu conhecimento de onde estão outras casas vem desta parte.
     */
    public void setUp(){
        for(int i = 0; i < TrilhaGame.getInstance().numberOfMen; i++){
            Man m = new Man(TrilhaGame.Color.values()[0]);
            m.setHouse(null);
            red.add(m);
        }
        for(int i = 0; i < TrilhaGame.getInstance().numberOfMen; i++){
            Man m = new Man(TrilhaGame.Color.values()[1]);
            m.setHouse(null);
            blue.add(m);
        }
